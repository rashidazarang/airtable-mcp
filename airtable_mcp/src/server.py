...
try:
    ...
    from mcp.server.base import MCPRequestContext, MCPToolDefinition
except ImportError:
...
@mcp.middleware
async def auth_middleware(context: MCPRequestContext, next):
    # Skip auth check for tool listing
    if context.operation == "list_tools":
